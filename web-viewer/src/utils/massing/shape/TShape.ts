...
export class TShape extends ShapeGenerator {
  generate(dimensions: ShapeDimensions): MassingShape {
    const dims = this.normalizeDimensions(dimensions);

    const armWidth = dims.armWidth ?? dims.width * 0.6;
    const armDepth = dims.armDepth ?? dims.height * 0.3;

    const topBar = createRectangle(armWidth, armDepth, 0, -dims.height / 2 + armDepth / 2);
    const stem = createRectangle(dims.width * 0.4, dims.height - armDepth, 0, 0);

    const outline = this.mergePolygons([topBar, stem]);
